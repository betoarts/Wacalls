...
import { fetchQuickReplyFile, listQuickReplies, markQuickReplyUsed, type QuickReply } from "@/services/quickReplies";
import { isDeletedMessage } from "@/lib/deleted-message";
import { resolveQuickReplyVars } from "@/lib/quick-reply-vars";
...
import { loadChatTranscripts, refKey, useTranscripts } from "@/stores/transcripts";
...
export const ChatView = ({ sessionId, chatJid, onStatusChange }: Props) => {
  ...
  const transcriptsByRef = useTranscripts((s) => s.byRef);

  // Carrega as transcrições já salvas deste atendimento (para busca e exibição).
  useEffect(() => {
    if (!sessionId || !chatJid) return;
    void loadChatTranscripts(sessionId, chatJid);
  }, [sessionId, chatJid]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void resolveLidPhone(sessionId, chatJid)
      .then((r) => {
        if (!cancelled && r?.phone) setLidPhone(r.phone);
      })
      .catch((err) => {
        console.error("Failed to resolve LID phone:", err);
      });
    return () => {
      cancelled = true;
    };
  ...
  // O picker aparece enquanto o texto for exatamente "/termo" (sem espaços).
  ...
  useEffect(() => { setQuickIdx(0); }, [text]);

  const applyQuickReply = useCallback(
    async (qr: QuickReply) => {
      const digits = (lidPhone ?? (chatJid ?? "").split("@")[0] ?? "").replace(/\D/g, "");
      const resolved = resolveQuickReplyVars(qr.content, {
        contactName: chat?.name ?? "",
        phone: digits ? `+${digits}` : "",
        agentName: myUser?.name || myUser?.email || "",
      });

      // Snippet com anexo: baixa o arquivo salvo e envia direto, usando o texto
      // resolvido como legenda. Sem anexo, mantém o comportamento antigo de
      // apenas preencher o campo de mensagem.
      if (qr.mediaUrl) {
        setSending(true);
        try {
          const file = await fetchQuickReplyFile(qr);
          await sendMedia(sessionId, chatJid, file, qr.mediaKind ?? "document", resolved);
          setText("");
          void markQuickReplyUsed(qr.id);
        } catch (e) {
          console.error("quick reply media send failed", e);
        } finally {
          setSending(false);
    ...
    [chat?.name, chatJid, lidPhone, myUser, sessionId],
  );
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (msg.includes("not paired")) {
        toast.error("Esta conexão do WhatsApp não está pareada. Acesse Conexões e escaneie o QR Code.");
      } else {
        toast.error("Falha ao enviar mensagem: " + (msg || "Erro desconhecido"));
      }
    } finally {
      setSending(false);
      requestAnimationFrame(() => messageInputRef.current?.focus());
  ...
  const handleFile = async (file: File | null, kind: "image" | "video" | "audio" | "document") => {
    ...
    try {
      ...
      if (msg.includes("not paired")) {
        ...
        toast.error("Falha ao enviar arquivo: " + (msg || "Erro desconhecido"));
      }
    } finally {
      setSending(false);
    }
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      rec.onstop = async () => {
        ...
        try {
          ...
          if (msg.includes("not paired")) {
            ...
            toast.error("Falha ao enviar áudio: " + (msg || "Erro desconhecido"));
          }
        } finally {
          setSending(false);
        }
      ...
      if (err?.name === "NotAllowedError") {
        alert(t("chat.errMicDenied", { defaultValue: "Permissão de microfone negada. Habilite no navegador para gravar áudio." }));
      } else if (err?.name === "NotFoundError") {
        alert(t("chat.errMicNotFound", { defaultValue: "Nenhum microfone encontrado." }));
      } else if (err?.name === "NotReadableError") {
        alert(t("chat.errMicInUse", { defaultValue: "O microfone está em uso por outro aplicativo." }));
      }
    }
  };
  ...
  const matchesSearch = (m: ChatMessage): boolean => {
    if (!search) return true;
    if ((m.body || "").toLowerCase().includes(search)) return true;
    const t = transcriptsByRef[refKey("message", m.id)];
    return !!t?.text && t.text.toLowerCase().includes(search);
  };
  const timeline: TimelineItem[] = [
    ...messages
      ...
      // Mensagens apagadas não entram na conversa: em vez de uma fila de
      // "Esta mensagem foi apagada", a thread mostra só o que continua valendo.
      .filter((m) => !isDeletedMessage(m))
  ...
  return (
    <div className="relative flex flex-1 flex-col">
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            <span className="truncate text-[11px] text-muted-foreground">
              {isGroup
                ? t("chat.peerGroup", { defaultValue: "Grupo" })
                : chatJid.endsWith("@lid")
                  ? (lidPhone ? formatPhone(lidPhone) : t("chat.hiddenNumber", { defaultValue: "Número oculto" }))
                  : formatPhone(chatJid)}
            </span>
          </div>
        </button>
      ...
      {showSearch && (
        <div className="flex items-center gap-2 border-b bg-card px-4 py-2">
          ...
          <input
            ...
            placeholder={t("chat.searchPlaceholder", { defaultValue: "Buscar em mensagens e transcrições de áudio…" })}
              ...
              return (
                <li key={item.key} className="flex flex-col">
                  ...
                  <MessageBubble
                    ...
                    onDelete={async (msg) => {
                      if (!chatJid) return;
                      if (!window.confirm(t("chat.confirmDeleteMessage", { defaultValue: "Apagar esta mensagem para todos?" }))) return;
                      try {
                        await deleteMessage(sessionId, chatJid, msg.id);
                      } catch (e) {
                        console.error("delete failed", e);
                        alert(t("chat.errDeleteMessage", { defaultValue: "Não foi possível apagar (mensagens antigas podem não permitir)." }));
                      }
                    }}
                  />
                </li>
        ...
        <div className="flex items-start gap-2 border-t bg-muted/40 px-3 py-2 text-xs">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-medium text-primary">
              {t("chat.replyingTo", { defaultValue: "Respondendo a {{name}}", name: replyTo.fromMe ? t("chat.youLower", { defaultValue: "você" }) : (replyTo.senderName || t("chat.contact", { defaultValue: "contato" })) })}
            </div>
      ...
      >
        ...
        <div className="relative">
          ...
          {showAttach && (
            <div className="absolute bottom-full left-0 mb-2 w-44 overflow-hidden rounded-lg border bg-popover shadow-md">
              ...
              <AttachItem icon={<Contact2 className="h-4 w-4" />} label={t("chat.attachContact", { defaultValue: "Contato" })} onClick={() => { setShowAttach(false); setShowContact(true); }} />
            </div>
          )}
        </div>
        ...
        <button
          type="button"
          title={signature.enabled ? t("chat.signatureOn", { defaultValue: "Assinatura ON (clique para editar)" }) : t("chat.signatureOff", { defaultValue: "Assinatura OFF" })}
          onClick={(e) => {
            if (e.shiftKey) {
        ...
        {recording ? (
          <div className="flex flex-1 items-center gap-2 rounded-md border bg-background px-3 py-1.5">
            ...
            <Button type="button" size="icon" variant="ghost" onClick={cancelRecording} aria-label={t("chat.cancelRecording", { defaultValue: "Cancelar gravação" })} title={t("common.cancel", { defaultValue: "Cancelar" })}>
              <X className="h-4 w-4" />
            </Button>
            <Button type="button" size="icon" onClick={stopRecording} aria-label={t("chat.sendAudio", { defaultValue: "Enviar áudio" })} title={t("chat.send", { defaultValue: "Enviar" })}>
          ...
          <>
            <div className="relative flex-1">
              <input
                ...
                onKeyDown={(e) => {
                  if (quickMatches.length > 0) {
                    if (e.key === "ArrowDown") {
                      e.preventDefault();
                      setQuickIdx((i) => Math.min(quickMatches.length - 1, i + 1));
                      return;
                    }
                    if (e.key === "ArrowUp") {
                      e.preventDefault();
                      setQuickIdx((i) => Math.max(0, i - 1));
                      return;
                    }
                    if (e.key === "Tab" || e.key === "Enter") {
                      e.preventDefault();
                      void applyQuickReply(quickMatches[quickIdx] ?? quickMatches[0]);
                      return;
                    }
                    if (e.key === "Escape") {
                      e.preventDefault();
                ...
                placeholder={noteMode ? t("chat.notePlaceholder", { defaultValue: "Nota privada (visível só para a equipe)" }) : t("chat.messagePlaceholder", { defaultValue: "Digite uma mensagem (Ctrl+V cola imagens/arquivos)" })}
              ...
              {quickMatches.length > 0 ? (
                <div className="absolute bottom-full left-0 right-0 z-30 mb-1 overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-lg">
                  <div className="flex items-center justify-between border-b px-2 py-1 text-[10px] uppercase tracking-wide text-muted-foreground">
                    <span>{t("chat.quickRepliesHint", { defaultValue: "Respostas rápidas · Tab para inserir" })}</span>
                    <button
                      type="button"
                      onMouseDown={(ev) => { ev.preventDefault(); setShowQuickManager(true); }}
                      className="flex items-center gap-1 rounded px-1 py-0.5 hover:bg-muted"
                    >
                      <Settings2 className="h-3 w-3" /> gerenciar
                    </button>
                  </div>
                  {quickMatches.map((qr, i) => (
                    <button
                      key={qr.id}
                      type="button"
                      onMouseDown={(ev) => { ev.preventDefault(); void applyQuickReply(qr); }}
                      onMouseEnter={() => setQuickIdx(i)}
                      className={`block w-full px-3 py-2 text-left ${i === quickIdx ? "bg-accent" : "hover:bg-accent/60"}`}
                    >
                      <div className="flex items-center gap-2 text-sm">
                        <span className="rounded bg-primary/10 px-1.5 py-0.5 font-mono text-xs text-primary">/{qr.shortcut}</span>
                      ...
                      <div className="truncate text-xs text-muted-foreground">{qr.content}</div>
                    </button>
                  ))}
                </div>
              ) : null}
              {quickMatches.length === 0 && suggestions.length > 0 && (
            ...
            {text.trim() ? (
              <Button type="submit" size="icon" disabled={sending} aria-label={t("chat.send", { defaultValue: "Enviar" })}>
                <Send className="h-4 w-4" />
              </Button>
            ) : (
              <Button type="button" size="icon" onClick={startRecording} aria-label={t("chat.recordAudio", { defaultValue: "Gravar áudio" })} disabled={sending || !canSend}>
        ...
        <aside className="absolute right-0 top-0 z-20 flex h-full w-80 flex-col border-l bg-card shadow-lg">
          <div className="flex items-center justify-between border-b px-3 py-2">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <History className="h-4 w-4" /> {t("chat.serviceLogs", { defaultValue: "Logs do atendimento" })}
            </div>
            <button
              type="button"
          ...
          <div className="flex-1 overflow-y-auto p-3 text-sm">
            {loadingHistory ? (
              <div className="text-xs text-muted-foreground">{t("common.loading", { defaultValue: "Carregando…" })}</div>
            ) : (() => {
              const KIND_LABEL: Record<string, string> = {
                created: t("chat.eventCreated", { defaultValue: "Conversa criada" }),
                waiting: t("chat.eventWaiting", { defaultValue: "Aguardando atendimento" }),
                opened: t("chat.eventOpened", { defaultValue: "Atendimento iniciado" }),
                closed: t("chat.eventClosed", { defaultValue: "Atendimento encerrado" }),
                requeued: t("chat.eventRequeued", { defaultValue: "Devolvido à fila" }),
                transferred: t("chat.eventTransferred", { defaultValue: "Transferido" }),
                call_incoming: t("chat.eventCallIncoming", { defaultValue: "Chamada recebida" }),
                call_outgoing: t("chat.eventCallOutgoing", { defaultValue: "Chamada realizada" }),
                call_answered: t("chat.eventCallAnswered", { defaultValue: "Chamada atendida" }),
                call_missed: t("chat.eventCallMissed", { defaultValue: "Chamada perdida" }),
                call_rejected: t("chat.eventCallRejected", { defaultValue: "Chamada rejeitada" }),
                call_canceled: t("chat.eventCallCanceled", { defaultValue: "Chamada cancelada" }),
                call_no_answer: t("chat.eventCallNoAnswer", { defaultValue: "Chamada não atendida" }),
                call_ended: t("chat.eventCallEnded", { defaultValue: "Chamada encerrada" }),
              };
              const displayUser = (id?: string, email?: string) => {
                if (id && userMap[id]) return userMap[id];
                if (email) return email.split("@")[0];
                return id || t("chat.system", { defaultValue: "Sistema" });
              };
              ...
              for (const c of closures) {
                ...
                items.push({
                  key: `c-${c.id}`,
                  ts: c.closedAt * 1000,
                  title: t("chat.eventClosed", { defaultValue: "Atendimento encerrado" }),
                  userId: c.userId,
              ...
              if (items.length === 0) {
                return (
                  <div className="text-xs text-muted-foreground">
                    {t("chat.noEvents", { defaultValue: "Nenhum evento registrado para esta conversa." })}
                  </div>
                );
              }
              return (
                  ...
                  {items.map((it) => (
                    <li key={it.key} className="rounded-md border bg-background p-2">
                      <div className="flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
                        <span>{new Date(it.ts).toLocaleString(i18n.language || "pt-BR")}</span>
                      ...
                      {it.reason !== undefined && (
                        <div className="mt-1 whitespace-pre-wrap text-xs">
                          {it.reason?.trim() ? (
                            <>
                              <span className="text-muted-foreground">{t("chat.reasonLabel", { defaultValue: "Motivo:" })} </span>
                              {it.reason}
                            </>
                          ) : (
                            <span className="italic text-muted-foreground">{t("chat.noReasonGiven", { defaultValue: "sem motivo informado" })}</span>
                          )}
                        </div>
                      )}
                    </li>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-md rounded-lg border bg-card p-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-2 flex items-center justify-between">
              <div className="text-sm font-semibold">{t("chat.editMessage", { defaultValue: "Editar mensagem" })}</div>
            ...
            />
            <div className="mt-1 text-[10px] text-muted-foreground">{t("chat.editWindowHint", { defaultValue: "O WhatsApp só permite editar mensagens com até 15 minutos." })}</div>
            <div className="mt-3 flex justify-end gap-2">
              <Button size="sm" variant="ghost" onClick={() => setEditing(null)}>{t("common.cancel", { defaultValue: "Cancelar" })}</Button>
              <Button
                size="sm"
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    alert(t("chat.errEditMessage", { defaultValue: "Não foi possível editar (talvez fora da janela de 15 minutos)." }));
                  }
                }}
              >
        ...
        <ForwardDialog
          ...
          onSubmit={async (targets) => {
            ...
            try {
              ...
              alert(t("chat.errForward", { defaultValue: "Não foi possível encaminhar." }));
            }
          }}
        />
      )}
...
const ContactForm = ({ onCancel, onSubmit }: { onCancel: () => void; onSubmit: (name: string, phone: string) => void | Promise<void> }) => {
  ...
  return (
    <div className="border-t bg-card px-3 py-2 text-xs">
      <div className="mb-1 flex items-center justify-between">
        <span className="font-medium">{t("chat.sendContact", { defaultValue: "Enviar contato" })}</span>
        <button className="text-muted-foreground hover:text-foreground" onClick={onCancel}>{t("common.close", { defaultValue: "fechar" })}</button>
      </div>
      <input
        value={q}
        onChange={(e) => setQ(e.target.value)}
        placeholder={t("chat.searchSavedContact", { defaultValue: "Buscar contato salvo (nome ou telefone)" })}
        className="w-full rounded-md border bg-background px-2 py-1 outline-none focus:ring-1 focus:ring-ring"
        autoFocus
      />
      <div className="mt-2 max-h-48 overflow-y-auto rounded-md border bg-background">
        {loading && <div className="px-2 py-2 text-muted-foreground">{t("chat.searching", { defaultValue: "Buscando…" })}</div>}
        {!loading && results.length === 0 && (
          <div className="px-2 py-2 text-muted-foreground">{t("chat.noContactsFound", { defaultValue: "Nenhum contato encontrado." })}</div>
        )}
      ...
      <div className="mt-2 flex justify-end">
        <Button size="sm" variant="ghost" onClick={onCancel}>{t("common.cancel", { defaultValue: "Cancelar" })}</Button>
      </div>
    </div>
  );
};
  ...
  if (activeCall) {
    const label =
      activeCall.status === "connected"
        ? t("chat.inCall", { defaultValue: "Em chamada" })
        : activeCall.status === "ringing"
          ? t("chat.calling", { defaultValue: "Chamando…" })
          : t("chat.connecting", { defaultValue: "Conectando…" });
    return (
      <div className="flex items-center gap-1.5">
        ...
        <Button
          size="sm"
          variant="destructive"
          title={t("chat.hangUp", { defaultValue: "Desligar chamada" })}
  ...
  return (
    <>
      <Button
        size="sm"
        variant="ghost"
        title={needsResolve ? t("chat.noRealNumber", { defaultValue: "Número real não disponível para este contato" }) : t("chat.call", { defaultValue: "Ligar" })}
        disabled={disabled}
        onClick={() => start.mutate({ phone: target, record: isCallRecordingEnabled(), video: false })}
      >
      ...
      <Button
        ...
        title="Vídeo chamada"
        disabled={disabled}
        onClick={() => start.mutate({ phone: target, record: isCallRecordingEnabled(), video: true })}
      >

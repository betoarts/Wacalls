...
import { looksDeleted } from "@/lib/deleted-message";
  ...
  if (filtered.length === 0) {
    return (
      <div className="p-4 text-sm text-muted-foreground">
        {t("chat.nothingHereYet", { defaultValue: "Nada por aqui ainda." })}
      </div>
    );
  }
  ...
  const handleAssign = (e: React.MouseEvent) => {
    ...
    void run("assign", async () => {
      try {
        ...
        onClick();
        toast.success(t("chat.assignedToast", { defaultValue: "Atendimento aceito" }));
      } catch (err) {
        toast.error(err instanceof Error ? err.message : t("chat.errAssign", { defaultValue: "Falha ao aceitar" }));
      }
    });
  };
  const handleClose = (e: React.MouseEvent) => {
    ...
    void run("close", async () => {
      try {
        ...
        toast.error(err instanceof Error ? err.message : t("chat.errClose", { defaultValue: "Falha ao finalizar" }));
      }
    });
  };
  const handleRequeue = (e: React.MouseEvent) => {
    ...
    void run("requeue", async () => {
      try {
        ...
        toast.error(err instanceof Error ? err.message : t("chat.errRequeue", { defaultValue: "Falha ao devolver" }));
      }
    });
  };
    ...
    <li>
      ...
      >
        ...
        <div className="relative shrink-0">
          ...
          {unread > 0 && (
            <span
              aria-label={t("chat.unreadCount", { defaultValue: "{{count}} não lida(s)", count: unread })}
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-end justify-between gap-2">
            <span className={`min-w-0 flex-1 truncate text-xs ${unread > 0 ? "text-foreground" : "text-muted-foreground"}`}>
              {looksDeleted(chat.lastKind, chat.lastMessage) ? (
                // A conversa esconde mensagens apagadas; a prévia não anuncia
                // um "Mensagem" que o operador não vai encontrar lá dentro.
                "—"
              ) : (
                <>
                  {chat.lastFromMe ? `${t("chat.you", { defaultValue: "Você" })}: ` : ""}
          ...
          <div className="ml-3 mr-1 flex shrink-0 items-center justify-center gap-1 self-center">
            {tab === "waiting" ? (
              <>
                ...
                <RowAction tone="success" label={t("chat.accept", { defaultValue: "Aceitar" })} busy={busy === "assign"} disabled={!!busy} onClick={handleAssign} square>
                  <Check className="h-3 w-3" strokeWidth={3} />
                </RowAction>
                <RowAction tone="lavender" label={t("chat.transfer", { defaultValue: "Transferir" })} onClick={handleTransfer} disabled={!!busy} square>
                  <ArrowLeftRight className="h-2.5 w-2.5" />
                </RowAction>
                <RowAction tone="dangerSolid" label={t("chat.finish", { defaultValue: "Finalizar" })} busy={busy === "close"} disabled={!!busy} onClick={handleClose} square>
              ...
              <>
                ...
                <RowAction tone="neutral" label={t("chat.requeue", { defaultValue: "Devolver para fila" })} busy={busy === "requeue"} disabled={!!busy} onClick={handleRequeue} square>

...
import { BarChart3, ChevronDown, ChevronsLeft, ChevronsRight, Contact2, HeartHandshake, History, Info, KanbanSquare, Layers, Maximize2, Megaphone, Menu as MenuIcon, MessageSquare, Minimize2, Palette, PhoneCall, Radio, Settings, ShoppingCart, Tag, Users2, Wifi, Workflow } from "lucide-react";
...
const AppShellInner = ({ children }: { children: ReactNode }) => {
  ...
  const isSuperAdmin =
    !!user && user.email.trim().toLowerCase() === "wacalls@admin.com";
  ...
  const principal: (NavItem & { perm: Permission; feat?: string })[] = [
    ...
    { to: "/flows", icon: Workflow, label: t("nav.flows", { defaultValue: "Fluxos" }), perm: "flows" },
  ...
  if (isAdmin) {
    ...
    aplicacoes.push({ to: "/settings/whitelabel", icon: Palette, label: t("nav.whitelabel", { defaultValue: "Whitelabel" }), perm: "chats" });
    aplicacoes.push({ to: "/settings/email", icon: Settings, label: t("nav.emailSettings", { defaultValue: "E-mail / SMTP" }), perm: "chats" });
  }
          ...
          <>
            {(!collapsed || forMobile) && (
              <div className="px-2 pb-2 pt-6 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground/80">
                {t("nav.admin", { defaultValue: "Administração" })}
              </div>
            )}
            <ul className="flex flex-col gap-1">
...
// Todos os "icon pills" do menu usam a cor primária (definida pelo
// Whitelabel) em vez de uma paleta arco-íris fixa por rota — assim a marca
// personalizada se reflete em 100% dos ícones, não só nos que caem no
// tom padrão.
const DEFAULT_TONE = { bg: "bg-nav-icon/15", fg: "text-nav-icon", ring: "ring-nav-icon/30" };
const toneFor = (_to: string) => DEFAULT_TONE;
  ...
  const iconPill = (highlighted: boolean) => (
    <span
      className={`grid place-items-center rounded-lg transition-colors ${collapsed ? "h-9 w-9" : "h-8 w-8"} ${
        highlighted ? "bg-nav-icon/20 ring-1 ring-nav-icon/40 text-nav-icon" : `${tone.bg} ${tone.fg} ring-1 ${tone.ring}`
      }`}
    >

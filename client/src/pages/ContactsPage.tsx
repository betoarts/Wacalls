...
export default function ContactsPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success(t("pages.contacts.createdToast", { defaultValue: "Contato criado" }));
      setForm({ sessionId: "", phone: "", name: "" });
      setOpenCreate(false);
      load();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t("pages.contacts.errCreate", { defaultValue: "Erro ao criar" }));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      toast.success(t("pages.contacts.updatedToast", { defaultValue: "Contato atualizado" }));
      closeEdit();
      load();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t("pages.contacts.errUpdate", { defaultValue: "Erro ao atualizar" }));
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (c: ContactRow) => {
    try {
      ...
      toast.success(t("pages.contacts.removedToast", { defaultValue: "Contato removido" }));
      load();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t("pages.contacts.errRemove", { defaultValue: "Erro ao remover" }));
    }
  };
  ...
  const startDirectCall = async (c: ContactRow) => {
    ...
    try {
      ...
      if (!phone || phone.length < 8) {
        toast.error(t("pages.contacts.errNoRealPhone", { defaultValue: "Número real indisponível para este contato" }));
        return;
      }
      ...
      toast.success(t("pages.contacts.callingToast", { defaultValue: "Chamando..." }));
    } catch (e) {
      const m = e instanceof Error ? e.message : t("pages.contacts.errCallFailed", { defaultValue: "Falha ao ligar" });
      if (m.includes("429")) toast.error(t("pages.contacts.errCallLimit", { defaultValue: "Limite atingido: chamadas simultâneas." }));
      else if (m.includes("503")) toast.error(t("pages.contacts.errNotPaired", { defaultValue: "WhatsApp não pareado." }));
      else toast.error(m);
    } finally {
      setCallingKey(null);
    }
    ...
    <AppShell>
      <div className="space-y-5 pb-12">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold tracking-tight">{t("pages.contacts.title", { defaultValue: "Contatos" })}</h2>
            <p className="text-sm text-muted-foreground">
              {t("pages.contacts.statsLine", {
                defaultValue: "{{total}} contatos · {{users}} pessoas · {{groups}} grupos",
                total: stats.total,
                users: stats.users,
                groups: stats.groups,
              })}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative min-w-[220px] flex-1">
            ...
            <Input
              className="pl-9"
              placeholder={t("pages.contacts.searchPlaceholder", { defaultValue: "Buscar por nome, telefone ou JID..." })}
              value={q}
          ...
          <Select value={kind || "all"} onValueChange={(v) => setKind(v === "all" ? "" : (v as KindFilter))}>
            ...
            <SelectContent>
              <SelectItem value="all">{t("pages.contacts.filterAll", { defaultValue: "Todos" })}</SelectItem>
              <SelectItem value="user">{t("pages.contacts.filterUsers", { defaultValue: "Pessoas" })}</SelectItem>
              <SelectItem value="group">{t("pages.contacts.filterGroups", { defaultValue: "Grupos" })}</SelectItem>
            </SelectContent>
          </Select>
        </div>
          ...
          <div className="overflow-hidden rounded-xl border bg-card">
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
                <tr>
                  <th className="px-4 py-2 text-left font-medium">{t("pages.contacts.colContact", { defaultValue: "Contato" })}</th>
                  <th className="px-4 py-2 text-left font-medium">{t("pages.contacts.colPhone", { defaultValue: "Telefone" })}</th>
                  <th className="px-4 py-2 text-left font-medium">{t("pages.contacts.colConnection", { defaultValue: "Conexão" })}</th>
                  <th className="px-4 py-2 text-left font-medium">{t("pages.contacts.colType", { defaultValue: "Tipo" })}</th>
                  <th className="px-4 py-2 text-right font-medium">{t("pages.contacts.colActions", { defaultValue: "Ações" })}</th>
                </tr>
              </thead>
              <tbody>
                {dedupedContacts.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-10 text-center text-muted-foreground">
                      {t("pages.contacts.noneFound", { defaultValue: "Nenhum contato encontrado" })}
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={`${c.sessionId}:${c.chatJid}`} className="border-t">
                      ...
                      <td className="px-4 py-2 font-mono text-xs">
                        {(() => {
                          ...
                          if (c.chatJid.endsWith("@lid")) return <span className="text-muted-foreground">{t("pages.contacts.resolving", { defaultValue: "Resolvendo…" })}</span>;
                          return "—";
                        })()}
                      </td>
                      ...
                      <td className="px-4 py-2">
                        {c.isGroup ? (
                          <Badge variant="secondary" className="gap-1">
                            <UsersIcon className="h-3 w-3" /> {t("pages.contacts.typeGroup", { defaultValue: "Grupo" })}
                          </Badge>
                        ) : (
                          <Badge variant="outline" className="gap-1">
                            <User className="h-3 w-3" /> {t("pages.contacts.typePerson", { defaultValue: "Pessoa" })}
                          </Badge>
                        )}
                      </td>
                        ...
                        <div className="flex justify-end gap-1">
                          {!c.isGroup && (
                            <Button
                              size="sm"
                              variant="ghost"
                              aria-label={t("pages.contacts.call", { defaultValue: "Ligar" })}
                              disabled={
      ...
      <Dialog open={openCreate} onOpenChange={setOpenCreate}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("pages.contacts.newDialogTitle", { defaultValue: "Novo contato" })}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-2">
              <Label>{t("pages.contacts.connectionLabel", { defaultValue: "Conexão" })}</Label>
              <Select value={form.sessionId} onValueChange={(v) => setForm({ ...form, sessionId: v })}>
                <SelectTrigger>
                  <SelectValue placeholder={t("pages.contacts.connectionPlaceholder", { defaultValue: "Selecione uma conexão" })} />
                </SelectTrigger>
                <SelectContent>
                  {sessions.map((s) => (
                    <SelectItem key={s.id} value={s.id}>
            ...
            <div className="space-y-2">
              <Label>{t("pages.contacts.phoneWithDdiLabel", { defaultValue: "Telefone (com DDI)" })}</Label>
              <Input
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={saving || !form.sessionId || !form.phone || !form.name}>
              ...
              {t("pages.contacts.create", { defaultValue: "Criar" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && closeEdit()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("pages.contacts.editDialogTitle", { defaultValue: "Editar contato" })}</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="space-y-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="outline" type="button" onClick={() => editFileRef.current?.click()}>
                      ...
                      {t("pages.contacts.choosePhoto", { defaultValue: "Escolher foto" })}
                    </Button>
                    {(editing.avatarUrl || editAvatarPreview) && (
                      ...
                      >
                        ...
                        {t("pages.contacts.removePhoto", { defaultValue: "Remover" })}
                      </Button>
                    )}
                  </div>
                  <p className="text-[11px] text-muted-foreground">{t("pages.contacts.photoHint", { defaultValue: "PNG, JPG ou WebP. Até 2 MB." })}</p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <Label>{t("pages.contacts.realPhoneLabel", { defaultValue: "Número real" })}</Label>
                <div className="flex items-center gap-2">
                  <Input
                    value={editPhone}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (editPhone) {
                        ...
                        toast.success(t("pages.contacts.phoneCopiedToast", { defaultValue: "Número copiado" }));
                      }
                    }}
                    aria-label={t("pages.contacts.copyPhone", { defaultValue: "Copiar número" })}
                  >
                    <Copy className="h-4 w-4" />
                  </Button>
                </div>
                <p className="text-[11px] text-muted-foreground">
                  {t("pages.contacts.realPhoneHint", { defaultValue: "Telefone exibido no discador e usado para ligações." })}
                </p>
              </div>

              <div className="space-y-2">
                <Label>{t("pages.contacts.jidLabel", { defaultValue: "Identificador do sistema (JID / LID)" })}</Label>
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      toast.success(t("pages.contacts.jidCopiedToast", { defaultValue: "JID copiado" }));
                    }}
                    aria-label={t("pages.contacts.copyJid", { defaultValue: "Copiar JID" })}
                  >
                    <Copy className="h-4 w-4" />
                  </Button>
                </div>
                <p className="text-[11px] text-muted-foreground">
                  {t("pages.contacts.jidHint", { defaultValue: "Gerado pelo WhatsApp. Somente leitura." })}
                </p>
              </div>
            </div>
          )}
      ...
      <ConfirmDialog
        ...
        title={t("pages.contacts.deleteTitle", { defaultValue: "Remover contato?" })}
        description={toDelete ? t("pages.contacts.deleteDescription", { defaultValue: "{{name}} será removido junto do histórico da conversa.", name: toDelete.name }) : undefined}
        confirmLabel={t("pages.contacts.remove", { defaultValue: "Remover" })}
        destructive
        onConfirm={() => {

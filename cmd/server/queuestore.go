...
func newQueueStore(ctx context.Context, db *sql.DB) (*queueStore, error) {
	...
	for _, stmt := range []string{
		...
		`ALTER TABLE queues ADD COLUMN distribution TEXT NOT NULL DEFAULT 'manual'`,
		`ALTER TABLE queues ADD COLUMN max_load INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE queues ADD COLUMN last_agent_id TEXT NOT NULL DEFAULT ''`,
	} {
...
func (s *queueStore) List(ctx context.Context, userID, tenantID string, isAdmin, superAdmin bool) ([]queueRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.Name, &r.Color, &r.OwnerID, &r.CreatedAt,
			&r.OrderBot, &ct, &rt, &r.RotationInterval, &r.RotationMode, &ar, &r.AgentID, &r.Greeting, &r.Distribution, &r.MaxLoad, &r.LastAgentID); err != nil {
			return nil, err
		}
		r.CloseTicket = ct != 0
		r.Rotation = rt != 0
...
func (s *queueStore) UpdateFull(ctx context.Context, id, name, color string, e queueExtras) error {
	...
	res, err := s.db.ExecContext(ctx, `UPDATE queues SET name = ?, color = ?,
		...
		auto_randomize = ?, agent_id = ?, greeting = ?, distribution = ?, max_load = ?
		WHERE id = ?`,
		strings.TrimSpace(name), color,
		e.OrderBot, qBool(e.CloseTicket), qBool(e.Rotation), e.RotationInterval, e.RotationMode,
		qBool(e.AutoRandomize), e.AgentID, e.Greeting, e.Distribution, e.MaxLoad, id)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
...
// SetLastAgent stores the round-robin cursor for the queue.
func (s *queueStore) SetLastAgent(ctx context.Context, queueID, userID string) error {
	_, err := s.db.ExecContext(ctx, `UPDATE queues SET last_agent_id = ? WHERE id = ?`, userID, queueID)
	return err
}

// Members returns the user IDs linked to the queue, in stable order.
func (s *queueStore) Members(ctx context.Context, queueID string) ([]string, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT user_id FROM user_queues WHERE queue_id = ? ORDER BY user_id`, queueID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()

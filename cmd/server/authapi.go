...
func (s *server) registerAuthRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /api/users/{id}/avatar", s.requireAdmin(s.handleUploadUserAvatar))
	mux.HandleFunc("DELETE /api/users/{id}/avatar", s.requireAdmin(s.handleDeleteUserAvatar))
...
func (s *server) handleUpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	var body struct {
		...
		SignatureEnabled *bool   `json:"signatureEnabled,omitempty"`
	...
	if body.SignatureEnabled != nil || body.Signature != nil {
		enabled := false
		if body.SignatureEnabled != nil {
			enabled = *body.SignatureEnabled
		}
		sig := ""
		if body.Signature != nil {
			sig = *body.Signature
		}
		_ = s.auth.UpdateSignature(r.Context(), id, enabled, sig)
	}
	w.WriteHeader(http.StatusNoContent)
}
...
func (s *server) handleUploadUserAvatar(w http.ResponseWriter, r *http.Request) {
	...
	for _, e := range []string{".png", ".jpg", ".webp", ".gif"} {
		_ = os.Remove(filepath.Join(dir, "user-"+id+e))
	}
	name := "user-" + id + ext
	full := filepath.Join(dir, name)
	dst, err := os.Create(full)
	...
	dst.Close()
	url := fmt.Sprintf("/api/media/avatars/%s?v=%d", name, time.Now().Unix())
	if err := s.auth.UpdateAvatar(r.Context(), id, url); err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(w, http.StatusOK, map[string]string{"avatarUrl": url})
...
func (s *server) handleDeleteUserAvatar(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.auth.UpdateAvatar(r.Context(), id, ""); err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": err.Error()})
		return
	}
	w.WriteHeader(http.StatusNoContent)

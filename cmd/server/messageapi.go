...
// onInboundMessage is wired by the server to cancel pending follow-ups when
// the customer answers. Nil until the stores are ready.
var onInboundMessage func(sessionID, chatJID string)
...
func (s *Session) handleWAMessage(evt *events.Message) {
	...
	if !row.FromMe && onInboundMessage != nil {
		onInboundMessage(s.id, row.ChatJID)
	}

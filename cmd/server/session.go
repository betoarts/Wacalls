...
func (s *Session) wireCall(cm *call.CallManager, callID string) {
	...
	cm.OnStateChange = func(c *call.CallInfo) {
		...
		if c.StateData.State == core.CallStateConnecting && c.Direction == core.CallDirectionIncoming {
			s.reg.armRecorder(c.CallID, 16000)
			s.startInboundFlowOnce(c.CallID, c.PeerJid, "connecting")
		}
		if c.StateData.State == core.CallStateActive {
			// Arm the server-side recorder for every answered call (manual
			// dialer, chat button or URA) so the audio shows up in Relatórios.
			s.reg.armRecorder(c.CallID, 16000)
...
func (s *Session) handleEvent(rawEvt any) {
	...
	case *events.Connected:
		...
		// Saudacao na conversa do proprio numero, so no primeiro Connected
		// depois do pareamento (controle em sessions.welcome_sent). O atraso
		// da tempo do cliente concluir o handshake antes do primeiro envio.
		go func() {

...
// OpenLoad counts the conversations currently assigned and open for a user.
func (s *chatMetaStore) OpenLoad(ctx context.Context, userID string) (int, error) {
	var n int
	err := s.db.QueryRowContext(ctx,
		`SELECT COUNT(1) FROM chats WHERE assigned_user_id = ? AND status = ?`,
		userID, ChatStatusOpen).Scan(&n)
	return n, err
}

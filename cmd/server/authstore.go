...
func (s *authStore) ListUsers(ctx context.Context) ([]UserRow, error) {
	...
	for i := range out {
		...
		qids, _ := s.QueuesFor(ctx, out[i].ID)
		out[i].QueueIDs = qids
		sids, _ := s.SessionsFor(ctx, out[i].ID)
		out[i].SessionIDs = sids
	}
	return out, nil
}
...
//
// Usa createAdminRaw em vez de Signup de proposito: o admin padrao precisa
// poder usar uma senha curta (o classico "admin"). A regra dos 8 caracteres
// continua valendo no cadastro e na troca de senha pelo painel.
func (s *authStore) SeedAdmin(ctx context.Context, email, password string) (bool, error) {
	...
	if err := s.createAdminRaw(ctx, email, password); err != nil {
		return false, err
	}
	return true, nil
}

// createAdminRaw cria um admin sem exigir tamanho minimo de senha. So e usado
// por operacoes LOCAIS de quem ja tem acesso ao servidor: o seed inicial e os
// comandos -reset-admin-* da linha de comando. Nada exposto na API chama isto.
func (s *authStore) createAdminRaw(ctx context.Context, email, password string) error {
	email = normalizeEmail(email)
	if !validEmail(email) {
		return ErrInvalidEmail
	}
	...
	id := newID()
	now := time.Now().Unix()
	if _, err := tx.ExecContext(ctx, `INSERT INTO users (id, email, password_hash, created_at, company_name, cpf, active, parent_id)
		VALUES (?,?,?,?,'','',1,'')`, id, email, string(hash), now); err != nil {
		if strings.Contains(err.Error(), "UNIQUE") {
			return ErrEmailTaken
		}
		return err
...
// SetPasswordRaw troca a senha sem a regra dos 8 caracteres. Mesma justificativa
// de createAdminRaw: uso local por linha de comando, nunca pela API.
func (s *authStore) SetPasswordRaw(ctx context.Context, userID, password string) error {
	if password == "" {
		return errors.New("password required")
	}
	hash, err := bcrypt.GenerateFromPassword([]byte(password), 12)

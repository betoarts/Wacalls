...
func newSessionStore(ctx context.Context, db *sql.DB) (*sessionStore, error) {
	...
	migrations := []string{
		...
		`ALTER TABLE sessions ADD COLUMN welcome_sent INTEGER NOT NULL DEFAULT 0`,
...
// welcomeSent informa se a saudacao de boas-vindas ja foi enviada para o
// proprio numero desta conexao. Volta a 0 quando o numero e desconectado, para
// que um novo pareamento receba a mensagem de novo.
func (s *sessionStore) welcomeSent(ctx context.Context, id string) (bool, error) {
	var n int
	err := s.db.QueryRowContext(ctx, `SELECT COALESCE(welcome_sent, 0) FROM sessions WHERE id = ?`, id).Scan(&n)
	if err != nil {
		return false, err
	}
	return n == 1, nil
...
func (s *sessionStore) setWelcomeSent(ctx context.Context, id string, sent bool) error {
	v := 0
	if sent {
		v = 1
	}
	_, err := s.db.ExecContext(ctx, `UPDATE sessions SET welcome_sent = ? WHERE id = ?`, v, id)
	return err
}

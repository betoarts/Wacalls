...
func newServer(ctx context.Context, dbPath, staticDir string, maxCalls int, log *slog.Logger) (*server, error) {
	...
	campaigns, err := newCampaignStore(ctx, db)
	if err != nil {
		return nil, err
	}
	...
	transcripts, err := newTranscriptStore(ctx, db)
	if err != nil {
		return nil, err
	}
	schedules, err := newScheduleStore(ctx, db)
	if err != nil {
		return nil, err
	}
	businessHours, err := newBusinessHoursStore(ctx, db)
	if err != nil {
		return nil, err
	}
	...
	onInboundMessage = func(sessionID, chatJID string) {
		...
		go srv.maybeSendOutOfHours(ctx, sessionID, chatJID)
		go srv.autoRouteChat(ctx, sessionID, chatJID)
	}
	srv.startScheduleWorker(ctx)

	// O disparador das campanhas roda em segundo plano, com o mesmo ciclo de
	// vida do servidor.
	srv.campaignRunner = newCampaignRunner(campaigns, bridge, mgr, log)
	srv.campaignRunner.Start(ctx)

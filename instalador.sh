...
executar_instalacao() {
  ...
  if projeto_tem_backend_go "${APP_DIR}"; then
    etapa_mariadb_redis
    # Whisper Bridge (STT): instala/garante o serviço local de transcrição e
    # configura WACALLS_STT_URL automaticamente — sem isso o botão
    # "Transcrever" do player de áudio fica bloqueado ("STT não configurado").
    etapa_whisper
    # Piper (TTS) segue desativado por padrão: o backend já cai para
    # espeak/nuvem sozinho quando WACALLS_TTS_URL não está definido.
  else
...
atualizar_via_zip() {
  ...
  if [ -z "${zip_file}" ] || [ ! -f "${zip_file}" ]; then
    ...
    log_info "Coloque o pacote (ex: vozzap-chat-completo.zip ou wacalls.zip) em /root e rode novamente."
    return 1
  fi
  log_info "Atualizando ${APP_NAME} a partir de: ${zip_file}"
  systemctl stop "${APP_NAME}.service" 2>/dev/null || true
  
  # Limpeza preventiva: removemos os builds antigos para garantir que o novo zip seja extraído e recompilado do zero,
  # mas preservamos dados essenciais.
  log_info "Limpando diretórios de build antigos para atualização limpa..."
  rm -rf "${APP_DIR}/dist" "${APP_DIR}/client/dist" 2>/dev/null || true
...
finalizar_atualizacao() {
  log_info "Limpando caches de build do frontend..."
  rm -rf "${APP_DIR}/dist" "${APP_DIR}/client/dist" 2>/dev/null || true
  
  log_info "Rebuild do frontend (isso pode demorar alguns minutos)..."
  ...
  if projeto_tem_backend_go "${APP_DIR}"; then
    ...
    # Garante o Whisper Bridge (STT) também em atualizações: se o serviço já
    # está instalado e ativo, etapa_whisper só confere/reaplica o .env; se
    # não tiver (instalação antiga, ou provisionada antes deste recurso
    # existir), instala do zero agora.
    etapa_whisper
  fi
...
etapa_whisper() {
  banner
  if systemctl is-active --quiet whisper-bridge.service 2>/dev/null; then
    log_info "[STT] Whisper Bridge já está instalado e ativo — só conferindo configuração."
    configurar_stt_env
    return 0
  fi
...
# Garante que o backend saiba usar o Whisper Bridge local, escrevendo
# WACALLS_STT_URL/WACALLS_STT_MODE no .env do app. Não sobrescreve se o
# usuário já tiver configurado um provedor de STT próprio (ex: OpenAI).
configurar_stt_env() {
  local env_file="${APP_DIR}/.env"
  mkdir -p "${APP_DIR}"
  touch "${env_file}"
  if ! grep -q '^WACALLS_STT_URL=' "${env_file}" 2>/dev/null; then
    {
      echo "# Whisper Bridge local (STT), provisionado pelo instalador"
      echo "WACALLS_STT_URL=http://127.0.0.1:5006/transcribe"
      echo "WACALLS_STT_MODE=multipart"
    } >> "${env_file}"
    log_ok "WACALLS_STT_URL configurado em ${env_file} (Whisper Bridge local)."
  else
    log_info "WACALLS_STT_URL já definido em ${env_file} — mantendo configuração existente."
  fi
  chmod 600 "${env_file}" 2>/dev/null || true
  [ "${APP_USER}" != "root" ] && chown "${APP_USER}:${APP_USER}" "${env_file}" 2>/dev/null || true
  if systemctl is-active --quiet "${APP_NAME}.service" 2>/dev/null; then
    log_info "Reiniciando ${APP_NAME} para aplicar configuração de STT..."
    systemctl restart "${APP_NAME}.service" || true
  fi
}
